...
export default function QuotePartDetail({ part, open, onClose, inCart, onAdd }: QuotePartDetailProps) {
      ...
      <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {aiData && <Badge variant="outline" className="gap-1 border-primary/40 text-primary"><Brain className="h-3 w-3" /> Pesquisa IA</Badge>}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Modelo: {part.machine_model || "—"} • Fabricante: {part.manufacturer || "—"}</p>
            <Badge variant={part.stock > 0 ? "secondary" : "destructive"} className="mt-2">{part.stock > 0 ? `${part.stock} unidades disponíveis` : "Indisponível"}</Badge>
          </div>
            ...
            <div className="space-y-3 border-t pt-4">
              {aiData.technical_description && (
                <div className="flex gap-2">
                  ...
                  <div>
                    <p className="text-xs font-semibold text-muted-foreground uppercase">Descrição Técnica</p>
              ...
              {aiData.probable_function && (
                <div className="flex gap-2">
                  ...
                  <div>
                    <p className="text-xs font-semibold text-muted-foreground uppercase">Função Provável</p>
              ...
              {aiData.compatible_machines && aiData.compatible_machines.length > 0 && (
                <div className="flex gap-2">
                  ...
                  <div>
                    <p className="text-xs font-semibold text-muted-foreground uppercase">Máquinas Compatíveis</p>
              ...
              {aiData.technical_specs && aiData.technical_specs.length > 0 && (
                <div>
                  <p className="text-xs font-semibold text-muted-foreground uppercase mb-1">Especificações</p>
          ...
          {similar && similar.length > 0 && (
            <div className="border-t pt-4">
              <p className="text-xs font-semibold text-muted-foreground uppercase mb-2">Peças Similares</p>
              <div className="grid grid-cols-2 gap-2">
                {similar.map((s: any) => (
                  <div key={s.id} className="bg-muted/50 rounded-lg p-2">
                    ...
                    <p className="text-xs text-primary font-medium">{s.stock} un.</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <Button className="w-full gap-2" onClick={onAdd} disabled={inCart || part.stock <= 0}>
            ...
            {inCart ? "Já adicionado à cotação" : "Adicionar à Cotação"}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
